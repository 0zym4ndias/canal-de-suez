...
int main()
{
    ...
    ifstream datosM;
    datosM.open(archSalida1);

    while (datosSuez >> ubi >> date >> hour >> buques)
    {

        registroVect registroV;
        registroV.ubi = ubi;
        registroV.fecha = date.substr(2, 4);
    }
    datosM.close();

    ifstream datosR;
    datosR.open(archSalida2);
