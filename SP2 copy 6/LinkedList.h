...
vector<registroVect> LinkedList<T>::busqueda(string ubi)
{
    if (size > 0)
    {
        ...
        int index = 0, cantM = 0, cantR = 0;

        while (curr1 != nullptr && curr1->getData().substr(0, 3) != ubi)
        {
            curr1 = curr1->getNext();
            index++;
        }
        ...
        while (curr1->getData().substr(0, 3) == ubi) // D9A
        {
            registroVect logRegistro; // Objeto
            int idx = curr1->getData.find("-");
            string mes = curr1->getData();
            string mes2 = mes.substr(idx + 1, 4);

            if (mes == curr1->getData().substr(idx + 1, 4))
            { // Valida Mes
                if (curr1->getData().substr(idx + 13, 4) == 'M')
                {
                    cantM++;
                }
                else
                ...
                logRegistro.mes = curr1->getData().substr(idx + 1, 4);
                logRegistro.year = curr1->getData().substr(idx + 5, 2);
                logRegistro.cantM = cantM;
                logRegistro.cantR = cantR;
            }
            else
            {
...
/*
while (curr1->getData().substr(0, 3) == ubi)
        {
            int idx = curr1->getData.find("-");
            string mes = curr1->getData().substr(idx + 1, 4);
            registroVect regitros;

            while (mes == curr1->getData().substr(idx + 1, 4))
            {
                regitros.mes = curr1->getData().substr(idx + 1, 3);
                regitros.year = curr1->getData().substr(idx + 5, 2);
                cantM++;
                string mes = curr1->getData().substr(idx + 1, 4);
                curr1 = curr1->getNext();
            }
            vectorF.push_back(regitros);
        }
        */