...
public:
    ...
    vector<registroVect> busqueda2(string ubi);
...
vector<registroVect> LinkedList<T>::busqueda(string ubi)
{
    ...
    //cout << curr1->getData().tripcar;

    while (curr1 != nullptr && curr1->getData().tripcar != ubi)
    {
        curr1 = curr1->getNext();
    }
    ...
    registroVect logSuez;

    while (curr1->getData().tripcar == ubi)
    {
        alerta++;
        if(mes == curr1->getData().mes){
            ...
            logSuez.cantMed = cantM;
            logSuez.cantRoj = cantR;
            if(alerta == 1){
                logSuez.mes = curr1->getData().mes;
                logSuez.year = curr1->getData().year;
            }
            if (mes != curr1->getNext()->getData().mes && curr1->getNext() != nullptr)
            {
                            vectorF.push_back(logSuez);
            }
        
        

        if (curr1->getNext()->getData().tripcar != ubi)
        {
            cout << "no es" << endl;
...
template <class T>
vector<registroVect> LinkedList<T>::busqueda2(string ubi)
{
    //vector<registroVect> vectorF;
    Node<T> *curr1 = head;
    vector<registroVect> vectorF;
