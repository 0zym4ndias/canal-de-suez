...
template <class T>
void LinkedList<T>::busqueda(string ubi){

    Node<T> *curr1 = head;
    vector<registroVect> vectorF;

    int index = 0, index2 = 0, cantM = 0, cantR = 0;

    while(curr1->getData().subtring(0,3) != ubi){
        
        curr1 = curr1->getNext();
        index++;
    }

    while(curr1->getData().substring(0,3) == ubi){
        
        registroVect regitros;
        regitros.fecha = curr1->getData().substr(1,4);
        cantM++;
        curr1 = curr1->getNext();
    }
